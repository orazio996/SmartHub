...
public class CalcolatoreConsumo extends CalcolatoreMetrica {

    public CalcolatoreConsumo(TipoMetrica tipoMetrica) {
        super(tipoMetrica);
    }
    ...
    /**
     * Calcola il consumo in kwh del target t.
     * Se il target t corrisponde ad un gruppo di 
     * dispositivi restituisce una metrica aggregata.
     */
    @Override
    protected double calcola(Target t, String param, List<Evento> eventi, FiltroTemporale filtro) {
        ...
        double consumoAggregato = 0.0;
        
        // metodo polimorfico
        List<Dispositivo> dispositivi = t.getDispositivi();

        for (Dispositivo d : dispositivi) {
        	consumoAggregato += calcolaConsumoSingolo(d, eventi, filtro);
        }
        return consumoAggregato;
    }
    
    

    /**
     * Calcola il consumo di ciascun dispositivo in base agli eventi in cronologia.
     */
    private double calcolaConsumoSingolo(Dispositivo d, List<Evento> eventi, FiltroTemporale f) {
        	...
        	if(e.getIdTarget().equals(d.getId())) {
	        	...
	        	if ("CONNESSIONE".equalsIgnoreCase(e.getTipo()) && statoPower.equalsIgnoreCase("ON")) {
	            	...
	            	if(orarioUltimoON != 0) {
		                ...
		                consumoKWh += (secondiAcceso / 3600.0) * d.getPotenza();
